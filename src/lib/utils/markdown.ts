...
function protectMath(markdown: string): { processed: string; placeholders: Placeholder[] } {
...
function restoreMath(html: string, placeholders: Placeholder[]): string {
	let result = html;
...
function admonitionPlugin(md: MarkdownIt) {
	...
	const colors: Record<string, { border: string; header: string; content: string }> = {
		note: {
	...
	md.core.ruler.after('block', 'admonitions', function(state) {
		...
		while (i < tokens.length) {
			...
			if (token.type === 'fence' && token.info.startsWith('ad-')) {
				...
				const adColors = colors[adType] ?? colors.note;
				let title = '';
				...
				admonitionToken.content = `
					<div class="admonition my-6 rounded-md overflow-hidden border ${adColors.border} shadow-lg">
						<div class="header flex items-center px-4 py-2 text-white ${adColors.header}">
							${icons[adType as keyof typeof icons] || icons.note}
							<span class="title font-medium capitalize">${title || adType}</span>
						</div>
						<div class="content ${adColors.content} p-4">
							${contentHtml}
						</div>
					</div>
				`;
...
export function structureTableOfContents(flatToc: TableOfContentsItem[]): TableOfContentsItem[] {
	const sections: TableOfContentsSection[] = [];
	let currentSection: TableOfContentsSection | null = null;
	let currentH1Section: TableOfContentsSection | null = null;
