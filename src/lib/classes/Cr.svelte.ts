import { patch, lineWidth } from '$stores';
...
export class VertexGroup {
    ...
    constructor(shapes: number[], molteplicity: number) {
        this.shapes = shapes;
        this.molteplicity = molteplicity;
        this.side = 25;
    }

    show(ctx: any, displayText: boolean, scale: number) {
        let baseAngle = 0;
        ...
        const lineWidthValue: number = get(lineWidth);
        if (lineWidthValue > 1) {
...
export class Cr {
    ...
    constructor(crString: string) {
        this.crString = crString;
        this.vertexGroups = this.parseCr(crString);
    }

    parseCr(crString: string): VertexGroup[] {
        ...
        let uniqueVertexGroups: VertexGroup[] = [];
    ...
    show(ctx: any, vertexIndex: number, displayText = true, scale = 1) {
        this.vertexGroups[vertexIndex].show(ctx, displayText, scale);
    }

    save(ctx: any, vertexIndex: number) {
        let scale = 3;
