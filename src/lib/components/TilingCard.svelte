<script>
	...
	let { name, cr, rulestring, groupId, onClick, golRules, imageUrl, dualImageUrl } = $props();
...
>
	<div class="flex flex-col gap-2">
		...
		<div class="relative w-full aspect-square bg-zinc-700/30 rounded-md flex items-center justify-center overflow-hidden border border-zinc-700/40 group-hover:border-green-500/50">
			<img src={imageSrc} alt={name} class="w-full h-full object-cover rounded-md group-hover:scale-105 transition-transform duration-300">
			{#if isDual}
