...
/** Convert short polygon format to display format (vertices as {x,y}). */
function polygonFromShort(enc: Record<string, unknown>): { type: string; n: number; vertices: { x: number; y: number }[] } {
	...
	const vertices = v ? v.map((p) => ({ x: p[0], y: p[1] })) : [];
	return {
...
async function loadManifest(
	...
	const manifest = await fetchPipelineJson(
		getExpandedSeedsManifestUrl(paramsFolder, k, m)
...
async function loadBatch(
	...
	const data = await fetchPipelineJson(
		getExpandedSeedsBatchUrl(paramsFolder, k, m, batchIndex)
	);
...
async function loadPageData(
	...
	if (polygonFilter) {
		...
		const displayData = pageData.map((item: any) => ({
			name: item.n ?? item.name ?? '',
			polygons: (item.p ?? item.polygons ?? []).map((p: Record<string, unknown>) => polygonFromShort(p)),
		}));
...
export async function load({ url, fetch }) {
	...
	for (const folder of supabaseFolders) {
		try {
			...
			const entries = structure.expandedSeeds ?? [];
	...
	// Polygon composition filter (Phase 3.5)
