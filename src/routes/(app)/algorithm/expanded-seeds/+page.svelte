<script lang="ts">
	...
	import { Minus, Plus, Puzzle, Camera, Layers, Grid2X2, List } from 'lucide-svelte';
	...
	$effect(() => {
		selectedCategories = data.selectedCategories?.length ? data.selectedCategories : categoryOptions.map((c) => c.id);
	...
	$effect(() => {
		headerStore.set({ title: 'Expanded Seeds', badge: String(data.totalItems), subtitle: null });
	});

	let columnsPerRow = $state(4);
	const COL_MIN = 2;
	const COL_MAX = 6;
	let extractCellsLoading = $state(false);
	let extractCellsError = $state<string | null>(null);
	let viewMode = $state<'grid' | 'list'>('grid');
	let selectedExpandedSeedNames = $state<Set<string>>(new Set());
	...
	const navigateTo = (
		...
		goto(`/algorithm/expanded-seeds?${params.toString()}`);
	};
	...
	function handleCardScreenshot(e: MouseEvent, filename: string) {
		const card = (e.currentTarget as HTMLElement).closest('.exp-card');
	...
	function initCanvas(canvas: HTMLCanvasElement, expandedSeed: any) {
		let resizeObserver: ResizeObserver;

		function render() {
			if (expandedSeed) drawExpandedSeed(canvas, expandedSeed);
		}
	...
	function drawExpandedSeed(canvas: HTMLCanvasElement, expandedSeed: any) {
		...
		if (!expandedSeed.polygons || expandedSeed.polygons.length === 0) return;

		const polygons = expandedSeed.polygons;
		...
		<div class="p-5 flex flex-col gap-6 lg:sticky lg:top-[65px] lg:max-h-[calc(100vh-65px)] lg:overflow-y-auto scrollbar-hide">
				...
				<div class="flex flex-wrap gap-1.5 mt-2.5">
					{#each data.kValues as k}
						...
						<button
							...
							onclick={() => navigateTo(k, null, 1, data.selectedParamsFolder ?? null, filterParams.categories.length > 0 ? filterParams : undefined)}
						>
							k={k}
						</button>
					{/each}
					{#if data.kValues.length === 0}
						<p class="text-xs text-zinc-600">No expanded seeds yet. Run the pipeline.</p>
					{/if}
				</div>
			</div>
				...
				<div class="border-t border-zinc-800 pt-5">
					...
					<div class="flex flex-wrap gap-1.5 mt-2.5">
						{#each mValuesForSelectedK as { m, count }}
							...
							<button
								class="km-btn {isActive ? 'km-btn-active' : ''}"
								onclick={() => navigateTo(data.selectedK, m, 1, data.selectedParamsFolder ?? null, filterParams.categories.length > 0 ? filterParams : undefined)}
							>
								<span>m={m}</span>
			...
			<div class="border-t border-zinc-800 pt-5">
				...
				<div class="flex flex-col gap-3 mt-2.5">
					<MultiSelect label="Category" options={categoryOptions} bind:selected={selectedCategories} />
					...
					{#if selectedCategories.length > 0}
						<button class="km-btn km-btn-active text-xs" onclick={() => handleFilterChange()}>
							Apply filter
						</button>
					{/if}
				</div>
	...
	<main class="flex-1 min-w-0">
		{#if data.selectedK === null || data.selectedM === null}
			<div class="flex flex-col items-center justify-center h-64 text-zinc-600">
				<Layers size={32} class="mb-3 opacity-50" />
				<p class="text-sm">Select k and m values to view expanded seeds.</p>
			</div>
		{:else if data.totalItems === 0}
			<div class="flex flex-col items-center justify-center h-64 text-zinc-600">
				<Puzzle size={32} class="mb-3 opacity-50" />
				<p class="text-sm">No expanded seeds found for k={data.selectedK}, m={data.selectedM}.</p>
				<p class="text-xs mt-2 text-zinc-500">Run the pipeline to generate expanded seeds.</p>
			</div>
		{:else}
			<div class="p-5 flex flex-col gap-4">
				<div class="flex flex-wrap items-center justify-between gap-3">
					<div class="flex items-center gap-3">
						...
						{#if data.expandedSeeds.length > 0}
							<div class="flex items-center gap-2">
								<span class="text-xs text-zinc-500">{selectedExpandedSeedNames.size} selected</span>
								<button
									...
									onclick={() => selectedExpandedSeedNames = new Set(data.expandedSeeds.map((es: any) => es.name ?? '').filter(Boolean))}
								>
									Select all
								</button>
								<button
									class="text-xs px-2 py-1 rounded border border-zinc-600/60 bg-zinc-800/60 text-zinc-400 hover:text-zinc-200 hover:bg-zinc-700/50"
									onclick={() => selectedExpandedSeedNames = new Set()}
								>
									Deselect all
								</button>
							</div>
					...
					<div class="flex gap-2">
						{#if extractCellsError}
							<span class="text-xs text-red-400">{extractCellsError}</span>
						{/if}
						<button
							...
							disabled={extractCellsLoading || !data.selectedParamsFolder || data.selectedParamsFolder === 'default'}
							onclick={async () => {
								const pf = data.selectedParamsFolder;
								if (!pf || pf === 'default' || data.selectedK === null || data.selectedM === null) return;
								extractCellsError = null;
								extractCellsLoading = true;
								try {
									await fetchPipelineWithProgress({
										url: '/api/pipeline/extract-translational-cells',
										body: {
											paramsFolder: pf,
											k: data.selectedK,
											m: data.selectedM,
										},
										title: 'Extract Translational Cells',
										initialMessage: 'Loading expanded seeds…',
									});
									await invalidate('app:polygons-filter');
									await goto(`/algorithm/tilings?polygons=${pf}&k=${data.selectedK}&m=${data.selectedM}`);
								} catch (e) {
									extractCellsError = e instanceof Error ? e.message : 'Unknown error';
								} finally {
									extractCellsLoading = false;
								}
							}}
						>
							{extractCellsLoading ? 'Extracting…' : 'Extract Translational Cells'}
						</button>
					</div>
				</div>
				...
				{#if viewMode === 'list'}
					<div class="flex flex-col gap-1.5">
						{#each data.expandedSeeds as expandedSeed, i}
							{@const globalIndex = (data.page - 1) * data.pageSize + i}
							<div
								class="exp-card exp-card-list group relative flex flex-row items-center gap-3 cursor-pointer"
								role="button"
								tabindex="0"
								onclick={() => {
									const next = new Set(selectedExpandedSeedNames);
									if (next.has(expandedSeed.name)) next.delete(expandedSeed.name);
									else next.add(expandedSeed.name);
									selectedExpandedSeedNames = next;
								}}
								onkeydown={(e) => {
							...
							>
								<input
									...
									checked={selectedExpandedSeedNames.has(expandedSeed.name)}
								...
								/>
								<span class="exp-index w-8 text-right shrink-0">{globalIndex + 1}</span>
								{#if browser}
									<div class="w-16 h-16 shrink-0 rounded overflow-hidden border border-zinc-700/50 bg-zinc-900">
										<canvas use:initCanvas={expandedSeed} class="block w-full h-full"></canvas>
									</div>
								{:else}
									<div class="w-16 h-16 shrink-0 rounded bg-zinc-800 flex items-center justify-center text-zinc-600 text-xs">—</div>
								{/if}
								<span class="text-zinc-500 exp-name truncate flex-1 min-w-0" title={expandedSeed.name}>{@html compactToHtml(compactSeedName(expandedSeed.name))}</span>
								<span class="text-zinc-600 text-xs shrink-0">{expandedSeed.polygons?.length ?? 0} polygons</span>
								{#if browser}
									<button
										type="button"
										...
										onclick={(e) => { e.stopPropagation(); handleCardScreenshot(e, `expanded-k${data.selectedK}-m${data.selectedM}-${globalIndex + 1}.png`); }}
										title="Screenshot"
				...
				>
					{#each data.expandedSeeds as expandedSeed, i}
						{@const globalIndex = (data.page - 1) * data.pageSize + i}
						<div
							class="exp-card group relative cursor-pointer"
							role="button"
							tabindex="0"
							onclick={() => {
								const next = new Set(selectedExpandedSeedNames);
						...
						>
							<div class="exp-card-header flex items-center gap-3" title={expandedSeed.name}>
								<input
									type="checkbox"
									class="h-4 w-4 rounded border border-zinc-600/60 bg-zinc-800/50 checked:bg-green-500/90 shrink-0"
									checked={selectedExpandedSeedNames.has(expandedSeed.name)}
								...
								/>
								<span class="exp-index">{globalIndex + 1}</span>
								<span class="text-zinc-500 exp-name truncate flex-1 min-w-0">{@html compactToHtml(compactSeedName(expandedSeed.name))}</span>
								<span class="text-zinc-600 text-xs shrink-0">{expandedSeed.polygons?.length ?? 0} polygons</span>
							</div>
							{#if browser}
								<canvas
									use:initCanvas={expandedSeed}
							...
							{#if browser}
								<button
									...
									onclick={(e) => handleCardScreenshot(e, `expanded-k${data.selectedK}-m${data.selectedM}-${globalIndex + 1}.png`)}
									title="Screenshot"
									aria-label="Take screenshot"
								>
									<Camera size={14} />
